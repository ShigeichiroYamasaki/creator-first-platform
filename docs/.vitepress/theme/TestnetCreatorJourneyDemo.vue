...
<template>
  <section class="creator-journey" aria-labelledby="creator-journey-title">
    ...
    <section class="panel"><h3>3. 音楽クリエータの利用資格を登録</h3><div class="status-grid"><div><span>実験参加の受付</span><strong>{{ participantRegistryReady ? '利用できます' : '準備中' }}</strong></div><div><span>運営の確認</span><strong>{{ creatorPreApproved ? '確認済み' : participantId !== zeroHash ? '期限切れ／停止' : '確認待ち' }}</strong></div><div><span>本人による登録</span><strong>{{ creatorParticipantRegistered ? '登録済み' : '未登録' }}</strong></div><div><span>練習用の手数料残高</span><strong>{{ initialFundingCompleted ? '受取済み' : '準備待ち' }}</strong></div></div><p v-if="!participantRegistryReady" class="safety">参加資格を記録する準備を進めています。時間をおいて表示を更新してください。</p><p v-else-if="!creatorPreApproved && !creatorParticipantRegistered" class="safety">この仮想通貨ワレットでは、運営の承認と初回POL受領を確認できません。招待登録に使ったものと同じ仮想通貨ワレットか確認してください。</p><p v-else-if="creatorPreApproved && !initialFundingCompleted" class="safety">運営の承認は確認できましたが、初回POLの準備が完了していません。招待ページの表示が更新されるまでお待ちください。</p><p v-else>本人が仮想通貨ワレットで確認し、音楽クリエータとしての実験参加を記録します。これは本人、権利者、報酬の受取人または配信許可の確認ではありません。</p><a v-if="!creatorPreApproved && !creatorParticipantRegistered" class="secondary link" :href="withBase('/demo/creator-participation')">実験参加の準備へ戻る</a><button v-if="creatorPreApproved && !creatorParticipantRegistered" class="primary" type="button" :disabled="!participantSelfRegistrationReady" @click="registerCreatorParticipant">音楽クリエータとして登録</button><p v-else-if="creatorParticipantRegistered"><strong>実験参加登録は完了しています。</strong></p></section>
