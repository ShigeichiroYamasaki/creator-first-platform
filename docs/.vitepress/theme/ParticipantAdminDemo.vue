...
function enrollmentStateLabel(invitation: Invitation) {
  ...
  return ({
    ...
    READY_AFTER_WALLET_CLAIM: '自動処理を待っています',
    READY_FOR_APPROVAL: 'オンチェーン承認を自動処理中',
    APPROVAL_SUBMITTED: 'オンチェーン承認確認中',
    APPROVED: '初回POL配布待ち',
    FUNDING_SUBMITTED: '初回POL配布確認中',
    APPROVAL_FAILED: 'オンチェーン承認を自動再試行中',
    FUNDING_FAILED: '初回POL配布を自動再試行中',
...
<template>
  <section class="participant-admin">
    ...
    <div class="panel">
      <h2>実験参加申請</h2>
      <p>「審査待ち」の申請を確認します。承認すると参加メールが1通送られ、本人がワレット登録を終えた後の参加承認と初回POL配布は自動で進みます。</p>
    ...
    <div class="panel">
      <h2>招待状況</h2>
      <p>承認済みの本人が仮想通貨ワレットを登録すると、参加承認と初回POL配布を自動実行します。一時的に失敗しても同じ参加者IDと操作IDで自動再試行するため、この一覧で追加操作は必要ありません。</p>
