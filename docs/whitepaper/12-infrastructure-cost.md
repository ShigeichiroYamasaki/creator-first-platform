...
ガス代支援は、資金フローと会計上、JPYC等によるサブスクリプション決済とは区別して記録する。一方、事業計画上の運営原価として固定月額料金へ織り込むことはできる。これはユーザへETH、POL等を販売・付与すること、またはユーザがそれらでサービス料金を支払ったと記録することを意味しない。ユーザには税込の月額総額と支援範囲・上限を事前表示し、個別ガス実費の事後請求は行わない。テスト系ではFaucet由来のテストネットガストークンだけを使用し、`MockJPYC`決済とSBT発行を無料で検証する。本番系ではSponsorship上限、対象操作、率制限、失敗時の再送、秘密鍵管理、会計処理および法務確認を承認済みポリシーとして定義する。

### 公開実験でのテストPOL配布

Polygon Amoyの公開実験は事前申請・招待制とし、匿名の公開Faucetとして運用しない。実験参加者は、招待ID、承認対象役割、同意版およびPolygon AmoyのチェーンIDを含むSIWEメッセージへ署名する。ゲートウェーが署名を検証した後、オフチェーンの運営ワーカーが上限付きのオンチェーン配布コントラクトを呼び出す。

初回はワレット残高を **0.02テストPOL** まで補い、承認済み操作に必要な追加配布では目標残高を最大 **0.05テストPOL** とする。ユーザと音楽クリエーターの役割を合算した一人当たり累積配布上限は **0.5テストPOL**、全参加者への日次配布上限は **1テストPOL** とする。既に目標残高がある場合は配布せず、参加者承認、ワレット重複確認、用途制限、操作ID、クールダウン、監査記録、残高監視および緊急停止を適用する。テストPOLには換金、返金、報酬または本番サービス上の権利を付与しない。
