...
flowchart TD
    ...
    USER -->|ファン登録| FAN[継続的な支援関係]
    USER -->|当月の配分指定| SUPPORT[換金不能な配分割合]
    FAN --> SUPPORT
    SUPPORT --> DIRECTED
    DIRECTED --> CREATORS
...
それぞれを分離することで、目的の異なる経済メカニズムを透明に設計する。ファン指定配分はユーザから音楽クリエーターへの自由送金ではなく、株式会社が受領した固定月額料金のうち、ガバナンスで上限を定めた分配可能額の配分指標とする。比率、同時指定数、未指定時の扱い、自己配分・関連当事者の制限および適用月をポリシー版ごとに公開する。

具体的な比率は運用実験と法務・会計レビューを経て決定する。説明用の例として、音楽クリエーター等への分配可能額を利用実績連動60%、ファン指定25%、成長・共通支援15%に分けることはできるが、この数値を確定条件または将来の報酬保証とはしない。税、公租公課、返金、権利保留、決済費用および運営原価を含む総収入全体との関係を同時に表示する。
...
確定収入にはサブスクリプション課金等を含める。ファン指定配分は新たな収入またはユーザから預かった送金原資として二重計上せず、分配可能額の内部配分属性として記録する。確定支出は少なくとも、利用実績連動分配、ファン指定分配、成長・共通支援分配、システム維持、納税・公租公課、プロモーション、コミュニティ運営を区別する。返金、取消、権利紛争、AML/CFT審査その他の保留額、または見積額を、確定した収支へ混在させない。
