...
### JASRAC・NexToneとの配信権利処理

Creator First Platformが日本国内で商用ストリーミングを行う場合、運営株式会社が配信事業者として、作品ごとの管理範囲を確認し、必要な利用許諾、利用実績報告および使用料支払を行う。JASRAC管理作品についてはJ-TAKT／J-NOTES、NexTone管理作品についてはPlayNを利用することを初期の業務経路とする。

両管理事業者が扱う作詞・作曲等の著作権と、音源の原盤権、実演家の権利、ジャケットその他の権利を混同しない。管理事業者から楽曲の利用許諾を得た場合でも、音源を配信するための原盤権者・実演家等からの許諾は別に確認する。
...
flowchart TB
    ...
    SELF[自己管理権利者]
    ANCHOR[権利・報告証跡<br/>スマートコントラクト]

    CREATOR -->|権利申告・原盤利用許諾| CORP
    CORP -->|審査・契約| REG
    REG -->|配信可能な権利状態| GATEWAY
    GATEWAY --> STREAM
    GATEWAY -->|再生記録| USAGE
    USAGE --> REPORT
    REG --> REPORT
    REPORT -->|指定形式で提出| JASRAC
    REPORT -->|指定形式で提出| NEXTONE
    REPORT -->|直接精算| SELF
    CORP -->|確認記録のハッシュ| ANCHOR
    REPORT -->|提出原本のハッシュ| ANCHOR
```

作品・権利登録台帳は、録音物を識別するISRC、楽曲を識別するISWC、JASRAC作品コード、NexTone作品コード、権利種別、持分、地域、利用形態、有効期間および確認証拠を版管理する。同じ作品について複数の権利者・管理主体・持分が存在できるものとし、事業者名だけで作品全体を単一の管理先へ分類しない。

再生実績はゲートウェーから改ざん検知可能な台帳へ記録し、適用した権利状態の版と結合する。報告生成サービスは共通の再生実績からJ-NOTES用、PlayN用および自己管理作品用の報告を分けて作成する。初期運用では、法人担当者が生成結果を確認して各公式システムへ提出し、受付結果と請求書を会計記録へ突合する。外部システムの画面スクレイピングや、共有パスワードによる自動操作は標準方式にしない。

スマートコントラクトには、権利を創設・移転する効力や、再生実績の真実性を自律的に判断する能力があるとは扱わない。公開チェーンへ記録するのは、承認済み権利スナップショット、提出報告ファイル、受付記録および支払証憑のハッシュ等に限定し、契約本文、個人情報、作品別の非公開収益明細は法人のアクセス制御された保管領域に置く。

著作権使用料、原盤・実演に対する契約上の支払、TuneCore等を通じた外部配信収益、CFP独自の音楽クリエーター報酬およびリスナー指定型配分は、別の根拠・台帳・勘定として扱う。ガバナンスは透明性基準やCFP独自報酬ポリシーを提案できるが、運営株式会社が負う利用許諾、期限内報告、使用料支払、配信停止その他の法的・契約上の義務を変更または停止しない。

詳細な実装判断は[ADR-0022 JASRAC・NexTone権利処理連携](/adr/ADR-0022-collective-rights-integration)に定める。JASRACの[商用配信手続](https://www.jasrac.or.jp/users/internet/procedure-business/)およびNexToneの[インターネット上での音楽利用手続](https://www.nex-tone.co.jp/copyright/users/int.html)を2026年9月2日に確認した整理であり、実運用開始時には最新の契約条件、報告書式、料金および管理範囲を運営株式会社と専門家が再確認する。
