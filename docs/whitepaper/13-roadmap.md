...
次のアカウント実証では、[ADR-0019](/adr/ADR-0019-jpki-passkey-wallet-binding-testnet)に従い、JPKI本人確認、FIDO2／WebAuthnパスキーおよびMetaMaskウォレットをアカウント・トラストサービスで分離して結合する。公開モードは実カードを使わないモックJPKIに限定し、パスキー登録はCFP管理の専用HTTPSドメイン、オンチェーン操作はPolygon Amoyで検証する。実在するJPKI接続は、認定事業者の試験環境、招待制パイロット、明示同意および個人情報保護レビューの後に実施する。
