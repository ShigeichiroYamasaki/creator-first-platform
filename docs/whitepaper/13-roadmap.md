...
現在のローカルモックはフェーズ 1のプレーヤー／ゲートウェイ再生境界と署名UIに加え、Hardhat 3上のテストネット専用MockJPYC決済、資金庫、サポーター SBT、音楽クリエーター登録台帳、二院制ガバナーおよびガバナンス対象デモポリシーを検証している。CFPはまだ運用を開始していないため、テストネット運用はPolygon Amoy（チェーンID `80002`）を起点として開始する。公開マニフェストに有効なものとして記録したAmoy上のコントラクトだけをテストネット正本とし、他ネットワークの実験的SBT、資格、投票または状態をCFP上の権利、履歴または移行対象として扱わない。公開テストユーザ利用フローからMockJPYC サブスクリプション、テスト音楽クリエーター利用フローから仮名音楽クリエーターと作品／権利自己申告コミットメント、ガバナンス画面から二院制の提案・投票境界を確認できる。
