---
description: 音楽ストリーミング市場の構造と、クリエイター還元・透明性・発見機会に関する課題の整理。
---
...
有料サブスクリプション型ストリーミングだけでも世界市場の **52.4%** を占め、サブスクリプションアカウントの利用者数は **8億3,700万人**に達している。
...
利用者は、個々の楽曲やアルバムを購入しなくても、大量の音楽へ合法的かつ即時にアクセスできるようになった。

クリエイターにとっても、物理媒体の製造や店舗流通を経ず、世界中の利用者へ作品を届けることが可能になった。
...
## 2.4 課題1 — 利用者が支払った価値の流れが見えにくい

一般的なストリーミングサービスでは、利用者は月額料金を支払う。

しかし、その利用者が支払った料金が、自分が聴いたクリエイターへそのまま配分されるとは限らない。

サービス全体の売上や再生シェア、権利関係、契約条件などを経て、複数の権利者へ分配される。

```mermaid
flowchart LR
    U[User<br/>Subscription] --> DSP[DSP]

    DSP --> POOL[Royalty Pool]

    POOL --> LABEL[Label / Distributor]
    POOL --> PUB[Publisher / CMO]
    POOL --> OTHER[Other Rights Holders]
...
クリエイターの視点でも、最終的な収益が複数の契約関係を経由するため、プラットフォーム上の利用実績と受取額の関係を理解することは容易ではない。
...
Creator First Platform が問題とするのは、**価値の流れと分配ルールが利用者やクリエイターから見えにくいこと**である。
:::
...
など、目的の異なる発見モードを利用者が選択できる設計を検討する。
...
しかし、そのルールがクリエイターの収益や発見可能性そのものを左右する場合、
...
一方で、収益分配やプロトコルの重要ルールについては、クリエイターと利用者が参加できる制度を設ける。
...
一方で、検証可能性を高めるために利用者の詳細な行動履歴を公開すれば、プライバシーを損なう。
...
Creator First Platform では、ゼロ知識証明などを利用し、利用者の詳細な履歴を公開せずに、分配に必要な利用実績を検証できる Usage Oracle の設計を検討する。
...
## 2.10 課題7 — クリエイター支援を「再生後の報酬」だけで考えない
...
などを組み合わせることで、発見前・成長途中のクリエイターも支援できる経済モデルを検討する。
...
flowchart LR
    ...
    G3[Generation 3<br/>Participation<br/>Creator First]
...
**クリエイター、利用者、コミュニティが、価値分配と発見とルール形成へ参加する。**
