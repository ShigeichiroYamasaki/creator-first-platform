...
STO、ステーブルコイン、国際的な資金移転に加え、ファン指定配分を扱う場合、AML/CFT、拡散金融および制裁対応が重要になる。典型的なリスクとして、犯罪収益を持つ者が複数の借名リスナーを用意し、共謀する音楽クリエーターへファン指定配分を集中させ、株式会社から支払われる正当な音楽報酬に見せかける行為が考えられる。

### 11.31.1 自由送金にしない構造対策

初期の本番候補では、ユーザが任意額を任意の音楽クリエーターへ直接送る投げ銭または送金機能を提供しない。
...
flowchart TD
    ...
    NET[分配可能額]
    ALLOC[上限付きファン指定配分]
    REVIEW[月次モニタリング / 人間の審査]
    CREATOR[確認済み音楽クリエーター]

    USER --> FEE --> CORP --> NET --> ALLOC --> REVIEW --> CREATOR
```

月額料金は利用規約に基づくサービス対価として株式会社が受領し、ファン指定は換金・譲渡・出金できない内部配分割合とする。指定可能割合、同時指定数、月次総額、最低配分単位および変更反映時期に上限を置く。ユーザ間送金、第三者への転送、換金可能な応援残高および別ワレットへの返金を設けず、返金が認められる場合は原則として元の支払経路へ戻す。

音楽クリエーターへの支払いは、株式会社が権利契約と配分規程に基づいて負う自己の報酬債務として月次で行う。ただし、契約上そのように記載するだけで資金移動、電子決済手段の管理・仲介その他の規制該当性を回避できるわけではない。資金の所有、スマートコントラクトの管理鍵、払戻し、ユーザの指図、手数料および実質的支配を含む事実関係について、本番開始前に金融規制専門家と確認する。

### 11.31.2 リスクに応じた本人・受取先確認

すべての音楽閲覧に金融取引と同じ本人確認を要求せず、行為とリスクに応じて認証レベルを分ける。一方、実価値を持つ資産による月額課金、ファン指定配分、報酬受領、受取先変更および高リスク操作には、次の統制を適用する。

- ユーザの一人性、本人確認状態、パスキーおよび本人が操作するワレットの結合
- 音楽クリエーター本人または法人、法人の実質的支配者、活動実態、権利状態および受取先の確認
- 本人または同一の実質的支配者への自己配分、関連当事者間配分および同一受取先の共有に対する申告・審査
- 報酬受取ワレットの事前登録、変更時の再認証および待機期間
- 採用する決済資産、発行者、コントラクトアドレス、ネットワークおよび取扱事業者の承認済み登録台帳による制限
- 制裁対象者、取引地域、取引目的およびウォレットのリスク情報の継続的な確認

JPKI等を利用する場合でも、マイナンバーそのものや秘密情報を収集せず、検証済み状態と必要最小限の証拠参照を保護されたオフチェーン環境で管理する。本人確認、取引モニタリングおよび制裁確認のために利用する個人情報は、利用目的、保存期間、アクセス権限、第三者提供、委託および異議申立て手続を明示する。

### 11.31.3 取引モニタリングと対応

固定の単一閾値だけに依存せず、顧客属性、商品、地域、取引形態、利用実績および過去の警告を組み合わせたリスクベースのモニタリングを行う。少なくとも次の事象を、直ちに不正と断定するのではなく人間の審査へ送る警告として扱う。

- 多数の新規または関連アカウントから一人の音楽クリエーターへの急激な集中
- 共通する資金源、端末、ネットワーク、認証手段または実質的支配者を持つ複数アカウント
- 聴取・コミュニティ活動と説明可能な関係がないファン指定だけの反復
- ファン登録・解除、受取先変更、返金要求または小口分割の不自然な反復
- 音楽クリエーターから元のユーザまたは関連者へ資金が還流する循環
- 制裁対象、盗難資金その他の高リスクな取引経路との関連

警告後は、追加確認、将来の報酬確定の一時保留、受取先変更の停止、取引拒否または法令・規約に基づく返金を段階的に行う。自動判定だけで資金を没収せず、理由、担当者、証拠、判断時刻、解除条件および異議申立てを監査可能に記録する。疑わしい取引の届出義務を負う特定事業者に該当する場合は、犯罪収益移転防止法その他の適用法令に従い主管行政庁へ届け出る。CFP自身が特定事業者に該当しない構成でも、登録済み決済事業者との責任分担、情報共有、照会および緊急連絡手順を契約で定める。

個別の抽出条件と閾値は、回避を助長しないよう一般公開しない。一方、監視対象の類型、保留・解除手続、集計された警告件数、誤検知率、処理期間およびポリシー版は、個人情報と捜査・セキュリティを害さない範囲で透明性報告に含める。

### 11.31.4 ガバナンスと株式会社の責任

配分上限、精算周期、受取先待機期間および監視方針はガバナンスの審議対象にできるが、法令に基づく本人確認、制裁対応、疑わしい取引の届出、証拠保全または取引停止を多数決で免除しない。緊急停止は限定された対象と期間で実施し、株式会社の法務・コンプライアンス責任者を含む複数承認、事後報告および復旧審査を必要とする。

公開テストネットでは`MockJPYC`およびテストPOLに実価値、償還請求権または本番報酬への移行権を持たせないため、現実の資金洗浄経路としては扱わない。ただし、一人性、指定上限、月次精算、警告、保留、解除、監査証跡および管理者権限分離を合成データで検証し、本番の統制設計に利用する。
...
| AML/CFT・拡散金融 | [金融庁：金融機関におけるマネロン・テロ資金供与・拡散金融対策](https://www.fsa.go.jp/policy/amlcftcpt/index.html) | リスク評価、継続的顧客管理、取引モニタリング、態勢整備 |
| 疑わしい取引 | [警察庁JAFIC：疑わしい取引の届出と届出先行政庁](https://www.npa.go.jp/sosikihanzai/jafic/todoke/todotop.htm) | 特定事業者の届出義務、届出先、記録・連携手続 |
| 経済制裁 | [財務省：経済制裁措置及び対象者リスト](https://www.mof.go.jp/policy/international_policy/gaitame_kawase/gaitame/economic_sanctions/list.html) | 資産凍結等対象者、更新情報、外為法上の確認 |
