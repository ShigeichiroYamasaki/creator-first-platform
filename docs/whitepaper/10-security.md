...
### 10.6.1 JPKI・パスキー・ウォレットの段階的結合

Creator First Platformでは、JPKI、FIDO2／WebAuthnパスキーおよびMetaMask等のウォレットを、一つの鍵または一つの本人識別子として扱わない。

- **JPKI:** 高保証の本人性確認と、必要な操作でのステップアップ認証
- **パスキー:** フィッシング耐性を持つ日常的なプラットフォーム認証
- **ウォレット:** オンチェーン操作、資産制御およびユーザ意思の署名
- **プラットフォームアカウント:** 各証明、同意、復旧および資格状態を結合する業務上の主体

```mermaid
flowchart LR
    JPKI[JPKI<br/>本人性]
    PASSKEY[パスキー<br/>日常認証]
    ACCOUNT[プラットフォームアカウント<br/>仮名ID・同意・復旧]
    WALLET[ウォレット<br/>オンチェーン意思表示]
    CREDENTIAL[用途別資格証明<br/>サポータ・議員等]
...
初回登録では、サーバ発行の一回限り結合トランザクションに、JPKIアダプターの有効性確認結果、WebAuthn登録結果およびウォレットのEIP-712結合署名を順に結び付ける。通常ログインと再生はパスキー認証済みセッションを主体とし、再生操作ごとにマイナンバーカードまたはウォレット署名を要求しない。主要ウォレット変更、アカウント復旧、一人性資格の再発行等では、JPKI再認証、パスキー、通知および待機期間によるステップアップを行う。

公開テストネットデモでは実在するマイナンバーカードまたは個人情報を扱わず、明確にモックと表示したJPKIアダプターを使用する。実カードを使う初期実証は20〜50人程度の招待制とし、デジタル庁のデジタル認証サービス認証APIを第一候補とする。利用申込み、事前準備契約、審査、接続試験、本契約、専用HTTPS環境、明示同意および個人情報保護レビューが完了するまでは実カードを扱わない。APIが無料であっても、承認、継続提供、SLAまたは将来の無償提供を保証されたものとは扱わない。

初期実証ではOpenID Connectの要求スコープを原則`openid`に限定し、署名APIを使用しない。日常利用はパスキーを主体とし、JPKIは初回の信頼登録、重複登録抑止が必要な実験、議員就任、高リスクな復旧等、目的を説明できる操作に限定する。署名、電子契約、最新基本4情報の取得等が必要になった段階で、認定プラットフォーム事業者との連携を別途評価する。

デジタル認証サービスが返すユーザ識別子はRP別のPPIDであり、削除、再登録、RP変更等で変わる可能性がある。CFPはランダムな内部アカウントIDを主キーとし、PPIDを恒久ID、公開識別子、ウォレット鍵のseedまたは永久的な一人性の唯一の根拠にしない。NPO法人による実証とCFP株式会社の本番サービスも別の管理主体・RPとして扱い、本番移行時には再同意、JPKI再認証、パスキー再登録およびウォレット再結合を行う。

WebAuthn資格情報はRP IDへスコープされるため、GitHub Pagesを恒久的な認証RPにせず、CFP管理の専用HTTPSドメインにパスキー登録・認証画面とアカウントサービスを置く。

マイナンバー、JPKI暗証番号、電子証明書原文・発行番号、氏名・住所等の基本情報、WebAuthn秘密鍵、生体情報およびJPKI事業者の生レスポンスは公開チェーンへ記録しない。オンチェーンへ置く場合も、用途別の資格コミットメント、ポリシー版および失効可能な最小限の参照に限定する。

JPKIによる認証は、音楽クリエーター適格性、著作権・原盤権、サポータ資格、議員適格性、分配請求権またはSTO上の権利を単独で成立させない。JPKIを利用できないユーザ、国外ユーザ、対応端末を持たないユーザにも、一般利用とコミュニティ参加の経路を残し、高保証が必要な役割には目的に応じた代替審査を用意する。SBTは公的身分証明ではなく、CFP所定の手続を特定時点で完了したことだけを表示する。詳細なテストネット境界、結合手続、復旧および受入条件は[ADR-0019](/adr/ADR-0019-jpki-passkey-wallet-binding-testnet)で定義する。
