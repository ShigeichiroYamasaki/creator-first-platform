...
公開テストネットデモでは実在するマイナンバーカードまたは個人情報を扱わず、明確にモックと表示したJPKIアダプターを使用する。実カードを使う実証は、招待制の専用環境、認定プラットフォーム事業者の試験接続、明示同意および個人情報保護レビューを成立条件とする。WebAuthn資格情報はRP IDへスコープされるため、GitHub Pagesを恒久的な認証RPにせず、CFP管理の専用HTTPSドメインにパスキー登録・認証画面とアカウントサービスを置く。
...
JPKIによる本人確認は、音楽クリエーター適格性、著作権・原盤権、サポータ資格、議員適格性、分配請求権またはSTO上の権利を単独で成立させない。詳細なテストネット境界、結合手続、復旧および受入条件は[ADR-0019](/adr/ADR-0019-jpki-passkey-wallet-binding-testnet)で定義する。
