...
本サイトは、稼働中の本番音楽配信サービス、決済サービス、DAOまたはSTOの案内ではありません。プロトコル仕様は草案であり、テストネットデモも現在準備中です。法務・金融・税務上の記述は個別案件への専門的助言ではありません。詳しくは[現在の状況](/status)と[テストネットデモ](/demo/)を確認してください。
:::
