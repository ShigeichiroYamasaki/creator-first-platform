...
この構成はIgnitionの`CreatorFirstTestnet`モジュールへ追加し、2026-08-26にモック検証者と受付台帳をPolygon Amoyへデプロイした。公開マニフェストには両アドレス、デプロイトランザクション、モックプロファイル登録トランザクションおよびソースコミットを記録する。これは接続境界の公開テストであり、透明型ゼロ知識証明方式の実証または本番適合を意味しない。
