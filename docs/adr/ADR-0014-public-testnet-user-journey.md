...
現行利用フローはユーザウォレットから直接トランザクションを送るため、Amoy POLをガスとして必要とする。これはADR-0008およびプロトコルが目標とするリレイヤー／ペイマスターによるガス Sponsored フローの完成形ではない。ガス代支援が実装されるまで、公開デモはこの制約を明示する。
...
- 静的サイトに秘密鍵を置けないため、公開デモではユーザがPolygon Amoy ガスを負担する。本番のゲートウェイ意思、リレイヤー、インデクサーおよびガス代支援を置き換えない。
...
- 直接ウォレットトランザクションはユーザにAmoy POLと複数回のウォレット確認を要求する。
...
2026-08-24時点で、利用フロー UI、マニフェスト検証、合成プレーヤー、一回限りのMockJPYC `claim()`、サポータSBT自己登録アダプター、EIP-712署名・発行・確定後表示、コントラクトテストおよび公開Site テストをリポジトリへ実装済みである。アダプターをPolygon Amoyへデプロイし、既存SBTプロキシへの接続と`RELAYER_ROLE`付与を公開RPCで検証した。公開マニフェストへ正規アドレスを記録したため、GitHub Pages反映後にSBT発行操作を利用できる。初期サポータポリシーは別途設定が必要であり、未設定時は一般サポータ階層を発行する。Polygonscan ソース検証、Bootstrap 役割分離、本番リレイヤー／ペイマスター、ゲートウェイ／インデクサー連携および本番ストリーミング認可は未実装である。この部分実装は本ADRの採用確定、セキュリティ監査または本番利用承認を意味しない。
