...
2026-08-24時点で、利用フロー UI、マニフェスト検証、合成プレーヤー、一回限りのMockJPYC `claim()`、サポータSBT自己登録アダプター、EIP-712署名・発行・確定後表示、コントラクトテストおよび公開Site テストをリポジトリへ実装済みである。既存公開構成はEthereum Sepoliaへデプロイ済みだが、新規アダプターは未デプロイで、マニフェストでは`null`としてfail closedにしている。アダプターのデプロイ、`RELAYER_ROLE`付与、初期ポリシー設定、公開記録更新が完了するまでSBT発行操作は無効である。Etherscan ソース検証、Bootstrap 役割分離、本番リレイヤー／ペイマスター、ゲートウェイ／インデクサー連携および本番ストリーミング認可も未実装である。この部分実装は本ADRの採用確定、セキュリティ監査または本番利用承認を意味しない。
