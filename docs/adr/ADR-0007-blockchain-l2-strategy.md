...
CFPはまだ運用を開始していないため、Polygon Amoyを最初かつ唯一の公開テストネット正本とする。テストネット上のSBT、資格、投票権、サブスクリプションおよびコントラクト状態は、チェーンID `80002`と公開マニフェストに登録された正規コントラクトアドレスが一致する場合だけ有効として扱う。それ以外のネットワーク上の実験的デプロイ、トークン、SBT、資格および状態はCFPの権利・特権・履歴を構成せず、移行、交換または権利承継の対象にしない。
...
アカウント抽象化、リレイヤー、ペイマスター、ガス代支援、Bundling等を利用できる。ユーザが確認する支払額はJPYC等の承認済み精算資産で固定し、運営が支払うネイティブ手数料をサブスクリプション決済として記録しない。具体方式、予算、権限分離および監査方針は[ADR-0021](./ADR-0021-relayer-gas-sponsorship.md)と各デプロイポリシーで決定する。
