...
テストユーザ登録はAlias表示と通知確認のUIを検証するTest-only プロフィールです。登録だけではプラットフォームアカウント、認証器、ウォレット連携、サブスクリプションまたはSBT資格を作成せず、再生認可も変更しません。別途、ADR-0019のアカウント・トラスト実証を完了した場合だけ、モックJPKI、パスキーおよびウォレットのテスト用結合状態を作成します。この状態も本番アカウントや本人確認ではありません。
...
`http://127.0.0.1:5173/creator-first-platform/demo/test-user-registration`を開きます。VitePressの`/api`プロキシだけが`127.0.0.1:8787`のゲートウェイへ接続し、画面はメディアアダプター URLまたは内部IDを受け取りません。独立したプレーヤーPWAを確認する場合は、2番目のコマンドを`npm run player:dev:gateway`へ置き換えます。
...
アカウント・トラスト実証では、上部でテストユーザを作成した後、モックJPKI確認、ブラウザのパスキー登録、MetaMaskのPolygon Amoy EIP-712結合署名を順に行います。結合署名は送金やトークン承認を含みません。
...
- モックJPKI、WebAuthnパスキーおよびMetaMaskを同じ短命結合IDへ固定する状態機械
- WebAuthnのorigin／RP ID／challenge／署名／ユーザ検証と資格情報カウンタのサーバ検証
- Polygon Amoy限定のEIP-712ウォレット結合と、SQLiteの結合・監査イベント記録
