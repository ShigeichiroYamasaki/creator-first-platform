---
title: 仮想通貨ワレット（MetaMask）の準備
description: 初めての実験参加者が、仮想通貨ワレットを安全に準備し、実際のお金を使わない練習用ネットワークへ接続する手順。
---

# 仮想通貨ワレット（MetaMask）の準備

このページでは、オレンジ色のキツネが目印の仮想通貨ワレット **MetaMask（メタマスク）** をパソコンへ追加し、Creator First Platform（CFP）の公開実験で使えるようにするまでを説明します。実際のお金を保管するためではなく、実験の操作を本人が確認するために使います。

公開実験に参加するには、先に運営への事前登録と招待が必要です。まだ招待を受けていない方は、[事前登録から参加までの流れ](/demo/)を確認してください。
...
## 4. デモ画面から練習用ネットワークへ切り替える

1. CFPの[音楽リスナーとして参加する画面](/demo/test-user-registration)を開く。
2. 「仮想通貨ワレットをつなぐ」を押す。
...
追加後、仮想通貨ワレットで接続先が「Polygon Amoy」と表示されれば完了です。これは実際のお金を使わない練習用ネットワークの固有名です。仮想通貨ワレットはサイトごとに異なる接続先を使えるため、CFPデモを開いた状態で表示を確認します。
...
- CFPデモ側の「練習用ネットワーク」が「接続済み」になっている
- 仮想通貨ワレットで操作を確認するとき、接続先がPolygon Amoyになっている

仮想通貨ワレットをつないだだけでは、残高の移動や応援の証明書の発行は行われません。その後の操作は一つずつ仮想通貨ワレットに確認画面が出るので、内容が分かる場合だけ承認します。
...
接続だけなら残高は不要ですが、実験内容を記録する操作には少量の練習用手数料残高（POL）が必要です。事前登録した実験参加者には、運営が必要な範囲で用意します。「POLを購入してください」という案内には従わず、運営へ連絡してください。

## 8. 次に試す操作

接続確認が終わったら、[公開実験の案内へ戻る](/demo/)か、[音楽リスナーとして参加する](/demo/test-user-registration)へ進みます。
