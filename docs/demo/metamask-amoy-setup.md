---
title: 財布アプリ（MetaMask）の準備
description: 初めての実験参加者が、財布アプリを安全に準備し、実際のお金を使わない練習用ネットワークへ接続する手順。
---

# 財布アプリ（MetaMask）の準備

このページでは、オレンジ色のキツネが目印の財布アプリ **MetaMask（メタマスク）** をパソコンへ追加し、Creator First Platform（CFP）の公開実験で使えるようにするまでを説明します。実際のお金を保管するためではなく、実験の操作を本人が確認するために使います。
...
追加後、財布アプリで接続先が「Polygon Amoy」と表示されれば完了です。これは実際のお金を使わない練習用ネットワークの固有名です。財布アプリはサイトごとに異なる接続先を使えるため、CFPデモを開いた状態で表示を確認します。
...
- 財布アプリで操作を確認するとき、接続先がPolygon Amoyになっている

財布アプリをつないだだけでは、残高の移動や応援の証明書の発行は行われません。その後の操作は一つずつ財布アプリに確認画面が出るので、内容が分かる場合だけ承認します。
