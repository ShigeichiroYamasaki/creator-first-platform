...
透明型ZKの2コントラクトとCFP-0002用のアダプター／デプロイファクトリーはローカル実装とIgnitionモジュールへの追加まで完了していますが、下記の公開Sepoliaマニフェストにはまだ含まれません。CFP-0002ポリシーはガバナンス実行時にだけ作成されます。アドレスとデプロイトランザクションを公開記録へ追加するまでは公開デプロイ済みと扱いません。
