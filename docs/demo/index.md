...
このデモには、現在GitHub Pagesだけで試せる**一般デモ**と、運営から招待された公開実験参加者が利用する**事前登録付きデモ**があります。

| 利用方法 | 対象 | 現在できること |
| --- | --- | --- |
| 一般デモ | 誰でも閲覧可能 | 仮名のテストユーザ登録、MetaMask接続、Polygon Amoyへの切替え、mockJPYC取得・利用承認・サブスクリプション、合成音源プレーヤー、サポータSBT |
| 事前登録付きデモ | 運営が事前承認した実験参加者 | 一回限りの招待メール、本人が選んだウォレットの署名確認、参加資格との結合、初回テストPOL配布、オンチェーン本人登録 |

::: info 事前登録付きデモの公開状況
招待URIの発行、メール送信、MetaMask署名による本人登録、参加者登録コントラクトは実装・自動テスト済みです。ただし、招待管理ゲートウェーと参加者登録コントラクトは公開環境へまだデプロイしていません。現在の公開GitHub Pagesでは一般デモを利用してください。
:::

### 一般デモを今すぐ試す手順

1. [MetaMaskとPolygon Amoyの準備手順](/demo/metamask-amoy-setup)を開き、MetaMaskをインストールしてPolygon Amoyを追加する。
2. 本番資産を入れていないテスト専用ウォレットへ切り替える。シードフレーズや秘密鍵はCFPのページへ入力しない。
3. [テストユーザ利用フロー](/demo/test-user-registration)を開き、個人情報を含まないAliasを登録する。
4. 「ウォレットを接続」からMetaMaskを接続し、ネットワークが`Polygon Amoy / 80002`であることを確認する。
5. MetaMaskに「資金を追加」と表示されても、本デモの登録操作では購入しない。トランザクションが必要な操作では、金銭的価値のないAmoyテストPOLだけをガス代に使う。
6. 画面の順番に従い、`2,000 tJPYCを取得`、サブスクリプションへの利用承認、サブスクリプション開始を行う。`tJPYC`は実在JPYCではない。
7. 合成音源プレーヤーを操作し、希望する場合だけ「サポータになってSBTを得る」を実行する。
8. MetaMaskに表示される送信先、ネットワーク、推定ガス代を毎回確認し、不明な署名やトランザクションは拒否する。

### 招待された実験参加者の登録手順

1. 運営が参加者をユーザ、音楽クリエーター、または両方の役割で事前登録する。この時点では参加者のEOAを運営へ知らせる必要はない。
2. 運営から届いた一回限り・期限付きの招待URIを開き、表示された参加者名、役割、有効期限を確認する。
3. 本人が利用するMetaMaskウォレットを接続し、ウォレット所有確認メッセージへ署名する。この署名は送金、トークン利用承認または課金ではない。
4. 利用条件とテストネット専用であることを確認し、「本人登録を完了」を実行する。
5. 運営が署名確認済みウォレットを参加者登録コントラクトへ承認し、必要最小限のテストPOLを配布するまで待つ。
6. [テストユーザ利用フロー](/demo/test-user-registration)へ移動し、オンチェーン本人登録、mockJPYCサブスクリプション、プレーヤー、SBTの順に試す。

招待メールの受信だけでは登録完了になりません。`メール送信 → 招待確認 → ウォレット署名 → 運営承認 → テストPOL配布 → オンチェーン本人登録`は別々の状態として表示されます。
...
| テストユーザサービス | GitHub Pagesにプロフィール／Polygon Amoy ウォレット／mockJPYC課金／合成プレーヤー利用フローを実装・公開。ウォレット未確定型の招待管理／メール送信／SIWE本人登録と参加者登録コントラクトはローカル実装・未デプロイ。ローカル同一オリジン構成にモックJPKI／WebAuthnパスキー／EIP-712ウォレット結合を実装。公開アカウントAPI、本番JPKI・認証器・復旧は未実装 |
