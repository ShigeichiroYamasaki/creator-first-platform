...
::: warning 現在の状態
テストネット専用の公開投票・二院制集計・承認後テスト証跡・タイムロックコントラクトとウォレット画面を実装しましたが、CFP候補bundleの自動生成、AI生成来歴、署名付きCI証明、SepoliaへのCFP-0002公開デプロイと会期設定は未完了です。議員登録はテスト運営者が行い、本人性、検証可能な抽選、秘密投票、異議申立て、本番権限移管または法的承認を実装したものではありません。
:::
...
3. Sepoliaでテスト運営者による議員資格登録、両院集計、レビュー証拠、CFP-0002ポリシー実行を接続する
4. 検証可能な抽選、異議申立て、シビル耐性、秘密投票およびアップグレード予行演習を追加する
