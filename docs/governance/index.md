...
> **CFP → 憲章・法務審査 → 仕様 → テストコード → 候補コード → 両院審議・投票 → 承認対象の再試験 → タイムロック → オンチェーン実行**

::: warning 現在の状態
テストネット専用の公開投票・二院制集計・承認後テスト証跡・タイムロックコントラクトとウォレット画面を実装しました。簡略議員登録アダプターをPolygon Amoyへデプロイし、公開デモ会期1を設定しました。本番候補の検証可能抽選コントラクトはローカル実装・自動テスト済みですが、実乱数、一人性、異議申立て、本番権限移管、CFP候補bundleの自動生成および署名付きCI証明は未完了です。
:::

[音楽クリエータ院議会](/demo/creator-house) · [ユーザ院議会](/demo/user-house) · [両院の統合画面](/demo/governance)
...
flowchart LR
    ...
    PRE[変更区分・憲章・法務審査]
    SPEC[仕様・テストコード]
    CANDIDATE[候補コード・隔離CI]
    CH[音楽クリエータ院議会]
    UH[ユーザ院議会]
    CV[音楽クリエータ院議会投票]
    UV[ユーザ院議会投票]
    JOINT{両院承認?}
    VERIFY[承認対象の再ビルド・再試験・監査]
    TIME[タイムロック]
    EXEC[オンチェーン実行]

    CFP --> PRE --> SPEC --> CANDIDATE
    CANDIDATE --> CH --> CV --> JOINT
    CANDIDATE --> UH --> UV --> JOINT
...
議員には、自然言語の要約だけでなく、要件とテストケースの対応、候補コードの差分、境界値、失敗時の挙動、シミュレーション結果を表示します。AIが生成した場合は、生成Tool・版、入力資料hashおよび人間によるレビュー状態も表示します。
...
flowchart TD
    ...
    REG --> EVIDENCE --> PRECHECK --> BUILD --> QV
    MEMBER --> QV
    QV --> BICAM
...
- **証拠保管庫:** CFP本文、憲章・法務審査、テスト、ソース、生成物、監査報告を内容アドレスで保存する。
- **憲章・法務審査台帳:** 投票前の適合性・実行可能性審査と根拠、適用版、担当権限を記録する。
- **隔離CI・ビルド証明:** デプロイ鍵を持たず、候補検証と両院承認後の最終再試験を別のクリーン環境で再現する。
...
1. ローカルHardhatでCFP-0002の事前審査、両院熟議証跡、両院投票、最終テスト証跡、タイムロック、限定デプロイを検証する（実装済み）
2. テストコード先行生成、候補コード生成、要件追跡表、隔離CI、再現可能ビルド証明を自動化する
3. Polygon Amoyで簡略資格に基づくテスト議員自己登録、両院集計、レビュー証拠、CFP-0002ポリシー実行を接続する
4. ローカル実装した検証可能抽選へ監査済み適格性・一人性・外部乱数を接続し、異議申立て、シビル耐性、秘密投票およびアップグレード予行演習を追加する
5. 独立監査と法務確認後に、CIと分離した本番タイムロック・デプロイ権限へ段階移行する
