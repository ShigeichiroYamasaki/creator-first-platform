...
::: warning 現在の状態
テストネット専用の公開投票・二院制集計・承認後テスト証跡・タイムロックコントラクトとウォレット画面を実装しました。簡略議員登録アダプターと本番候補の検証可能抽選コントラクトはローカル実装・自動テスト済みですが、Sepoliaへの議員登録アダプター、会期・議員・CFP提案の設定、実乱数、一人性、異議申立て、本番権限移管、CFP候補bundleの自動生成および署名付きCI証明は未完了です。
:::
...
3. Sepoliaで簡略資格に基づくテスト議員自己登録、両院集計、レビュー証拠、CFP-0002ポリシー実行を接続する
