...
::: warning 現在の状態
このページは制度とユーザインターフェースの設計です。公開テストネットの投票・タイムロックコントラクトは未実装であり、表示例を実際の投票として扱うことはできません。
:::
...
1. オフチェーン CFPと両院画面で提案・審議・模擬QVを検証する
2. Sepoliaで議員資格 snapshot、QV集計、Bicameral approval、タイムロックを接続する
3. 監査用実行マニフェストとアップグレード dry-runを追加する
