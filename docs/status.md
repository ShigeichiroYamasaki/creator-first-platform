...
| ADR | 提案 | 17件の設計判断案 | レビュー、採否決定、状態更新 |
...
| テストネットデモ | 公開テストネット利用フロー部分稼働 | [テストユーザ利用フロー](/demo/test-user-registration)のプロフィール／ウォレット／mockJPYC／合成プレーヤー／サポータSBT UIとSepolia登録アダプター、[テスト音楽クリエーター利用フロー](/demo/creator-workspace)のプロフィール／ウォレット／音楽クリエーター・リリースコミットメント | 初期サポータポリシー、公開ウォレット E2E確認、ゲートウェイ公開と認可接続、音楽クリエーターBFF／インデクサー、本番リレイヤー／ペイマスター、セキュリティレビュー |
...
| スマートコントラクト | 既存系はEthereum Sepoliaへデプロイ済み、透明型ZK境界は公開デプロイ前、全体監査前 | [公開アドレス／ソースコミット](/demo/testnet-contracts#public-deployment)、MockJPYC、サブスクリプション、資金庫、一般／初期サポーター SBT、UUPS プロキシ、音楽クリエーター登録台帳、二院制ガバナー、デモポリシー、公開RPC検証。透明型ZKの交換可能インターフェース、明示的モック検証者、チェーン拘束受付台帳はローカル実装・テスト済み | 透明型ZK境界のSepoliaデプロイと公開マニフェスト更新、実証用UI、Etherscan ソース検証、役割分離、ゲートウェイ／インデクサー接続、脅威モデル、独立監査 |
...
| DAOガバナンス | CFP識別子・改訂結合を備えた二院制ガバナーV2とデモポリシーをEthereum Sepoliaへ公開デプロイ済み。会期・議員・CFP提案の公開運用とCFP記録ゲートの正式運用は未開始 | [二院制議会・二次投票・コントラクト変更手続](/governance/)、[音楽クリエータ院議会](/demo/creator-house)、[ユーザ院議会](/demo/user-house)、[CFP文書・議事録管理](/proposals/record-management)、[ガバナーV2とデモポリシーの公開アドレス](/demo/testnet-contracts#public-deployment)、[CFP-0002](/proposals/CFP-0002-early-supporter-one-year-rule)、CFP専用承認投票、改訂重複拒否、会期別同量投票クレジット、投票前適合性審査、両院別集計、レビュー、P2タイムロック、完全一致デプロイの自動テスト | テスト会期・両院議員・CFP提案・事前審査・両院熟議証拠の登録、重大論点の解決、正式な三憲章、実在する審査者・議員・熟議、確認済み議事録、検証可能な抽選、本人性・重複防止、秘密投票、異議申立て、理由付き差し戻し、本番権限移管、監査 |
