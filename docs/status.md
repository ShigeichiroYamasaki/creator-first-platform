...
| テストネットデモ | 公開テストネット利用フロー部分稼働 | [テストユーザ利用フロー](/demo/test-user-registration)のプロフィール／ウォレット／mockJPYC／合成プレーヤー／サポータSBT UIとPolygon Amoy登録アダプター、[テスト音楽クリエーター利用フロー](/demo/creator-workspace)のプロフィール／ウォレット／音楽クリエーター・リリースコミットメント | 初期サポータポリシー、公開ウォレット E2E確認、ゲートウェイ公開と認可接続、音楽クリエーターBFF／インデクサー、本番リレイヤー／ペイマスター、セキュリティレビュー |
...
| スマートコントラクト | 既存系と簡略議員登録はPolygon Amoyへデプロイ済み、検証可能抽選・透明型ZK境界は公開デプロイ前、全体監査前 | [公開アドレス／ソースコミット](/demo/testnet-contracts#public-deployment)、MockJPYC、サブスクリプション、資金庫、一般／初期サポーター SBT、UUPS プロキシ、音楽クリエーター登録台帳、二院制ガバナー、デモポリシー、簡略議員登録アダプター、公開デモ会期1、公開RPC検証。本番候補の適格集合・外部乱数・部分Fisher–Yates抽選・当選証明・補欠順位コントラクト、透明型ZK境界はローカル実装・テスト済み | 公開デモ会期の運用、適格性・一人性・実乱数・異議申立て接続、透明型ZK境界のPolygon Amoyデプロイ、Polygonscan ソース検証、役割分離、脅威モデル、独立監査 |
