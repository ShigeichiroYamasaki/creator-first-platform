...
| スマートコントラクト | 既存系はEthereum Sepoliaへデプロイ済み、議員登録・検証可能抽選・透明型ZK境界は公開デプロイ前、全体監査前 | [公開アドレス／ソースコミット](/demo/testnet-contracts#public-deployment)、MockJPYC、サブスクリプション、資金庫、一般／初期サポーター SBT、UUPS プロキシ、音楽クリエーター登録台帳、二院制ガバナー、デモポリシー、公開RPC検証。簡略議員登録アダプター、本番候補の適格集合・外部乱数・部分Fisher–Yates抽選・当選証明・補欠順位コントラクト、透明型ZK境界はローカル実装・テスト済み | 議員登録アダプターのSepoliaデプロイ、テスト会期設定、適格性・一人性・実乱数・異議申立て接続、透明型ZK境界のSepoliaデプロイ、Etherscan ソース検証、役割分離、脅威モデル、独立監査 |
...
| DAOガバナンス | 二院制ガバナーV2とデモポリシーはSepoliaへデプロイ済み。簡略議員登録と本番候補の検証可能抽選はローカル実装・自動テスト済みだが未デプロイ | [二院制ガバナーとデモポリシーの公開アドレス](/demo/testnet-contracts#public-deployment)、[音楽クリエータ院議会](/demo/creator-house)、[ユーザ院議会](/demo/user-house)、[検証可能抽選ADR](/adr/ADR-0002-verifiable-sortition)、CFP専用承認投票、テスト資格別自己登録、両院別適格集合固定、外部乱数要求、重複なし抽選、Merkle当選証明、同一会期の一人一議席、順序付き補欠のコントラクト試験 | 議員登録アダプターのSepoliaデプロイと会期設定、本番適格性・一人性・監査済みVRF／乱数ビーコン・異議申立ての接続、役割分離、正式な三憲章、実在する審査者・議員・熟議、秘密投票、本番権限移管、独立監査 |
