...
`GATEWAY_PARTICIPANT_ENROLLMENT_AUTO_PROCESS=true`では、運営承認によって自動処理を予約し、招待に結び付いたSIWE署名と本人登録が確定した直後に、参加者登録運営ワーカーを起動します。参加者はPOLを持たずにオンチェーン承認と初回POL配布を受けられます。処理は参加者IDと操作IDで冪等化され、失敗・中断状態は定期照合によって自動再試行されます。管理画面からの手動実行は通常経路に含めません。ローカル開発の既定値は`false`です。
