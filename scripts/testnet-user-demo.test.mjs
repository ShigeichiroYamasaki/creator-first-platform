...
test('ships an active Sepolia manifest with reviewed addresses and source commit', async () => {
  ...
  assert.equal(parsed.sourceCommit, 'b0bfcb73d453d970e0a5c1c432abb9abc6e0d341')
  ...
  assert.equal(parsed.contracts.supporterRegistrationAdapter, null)
  ...
  assert.equal(hasActiveSupporterRegistration(parsed), false)
})
