...
test('separates participant preparation from post-POL service experiences', async () => {
  ...
  assert.match(listenerJourney, /class="support-target-list"[^>]+tabindex="0"/)
  assert.match(listenerJourney, /\.support-target-list\{[^}]*max-height:[^}]*overflow-y:auto/)
  assert.match(listenerJourney, /grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/)
  ...
  assert.match(listenerJourney, /トークン発行を送信済み/)
  assert.doesNotMatch(listenerJourney, /デジタル証明書|応援の証明書|証明書を受け取る/)
  assert.match(listenerExperience, /サポータートークン（SBT）/)
