...
- **REQ-GOVERNANCE-051:** A sponsored ballot MUST be authorized by the member's EIP-712 signature bound to the Chain ID, Governor address, Proposal ID, Session ID, committed House, member wallet, signed intensity, member nonce and short deadline; the Governor MUST recover and record the member rather than the transaction sender as the voter.
- **REQ-GOVERNANCE-052:** The Governor MUST reject an expired sponsored ballot, reused or non-current member nonce, changed ballot field, signer mismatch, House or Session mismatch, missing CFP binding and submission by an account without the purpose-limited Relayer Role.
- **REQ-GOVERNANCE-053:** Sponsored and direct ballots from the same member MUST use the same canonical membership, replacement, quadratic cost, Session budget and separate-House aggregation rules. Paying gas MUST NOT grant the Relayer Voice Credit, House membership, ballot authorship or authority to alter signed intent.
- **REQ-GOVERNANCE-054:** Participant enrollment, Supporter SBT relay, governance relay, review, guardian and deployment keys MUST be separately revocable. Relayer availability, role and gas balance MUST be health-checked, while relay failure MUST NOT permit an operator to fabricate or silently substitute a ballot.
...
The testnet sponsored-voting profile uses `castCfpApprovalVoteBySig`. The member signs the exact EIP-712 `CfpBallot`; a dedicated Gateway relayer validates the signature and current on-chain House and nonce, then pays POL to submit it. The Governor independently repeats signature, nonce, deadline, Session, House, CFP binding, membership and quadratic-accounting checks. The signature and resulting public ballot remain attributable to the member. This is gas sponsorship, not delegation and not ballot privacy. Polygon Amoy Governor V3, its purpose-limited relayer role and the replacement public Session are deployed and funded; the Gateway MUST still fail closed until its reviewed runtime secret, same-origin health result, manifest and public wallet end-to-end test all match that deployment.
...
- Contract and Gateway tests for REQ-GOVERNANCE-051–054 recover the member from the exact typed ballot, preserve direct-vote accounting and member POL, reject replay, expiry, field substitution, wrong House and unauthorized relayers, and fail closed when relay role or balance is unavailable.
